...
export default function AdminPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      {/* Top bar */}
      <header className="sticky top-0 z-30 flex items-center justify-between px-6 py-4 bg-[#050507]/95 backdrop-blur border-b border-zinc-900">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={save}
            disabled={saving}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg font-heading font-black text-xs uppercase tracking-wider transition-all cursor-pointer ${saved ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/30" : "bg-brand-cyan/20 text-brand-cyan border border-brand-cyan/30 hover:bg-brand-cyan/30"}`}
          >
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar */}
        <nav className="w-52 shrink-0 border-r border-zinc-900 pt-6 px-3 space-y-1">
          {TABS.map(({ key, label, icon: Icon }) => (
            <button
              key={key}
              onClick={() => setTab(key)}
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left font-heading font-bold text-xs uppercase tracking-wider transition-all cursor-pointer ${tab === key ? "bg-brand-purple/20 text-brand-purple border border-brand-purple/30" : "text-zinc-500 hover:text-zinc-300 hover:bg-zinc-900"}`}
            >
